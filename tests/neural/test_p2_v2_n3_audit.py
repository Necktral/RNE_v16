...
def receipt(arm):
    return {
        ...
        "raw_candidate_ids": [f"m{i}" for i in range(12)],
        "raw_candidate_scores": [float(i) for i in range(12)],
        "raw_candidate_scales": [s for _ in ("a", "b") for s in ("micro", "micro", "meso", "meso", "macro", "macro")],
        "canonical_order_ids": [f"m{i}" for i in range(12)],
        "arm_order_ids": [f"m{i}" for i in range(12)],
        "raw_candidate_structures": [
            {"intervention": iv, "relation_kind": relation}
            for iv in ("a", "b") for _scale in ("micro", "meso", "macro")
            for relation in ("support", "contradiction")
        ],
        "exposed_memory_ids": ["m0"], "exposed_memory_sha256": "dummy",
...
def test_auditor_detects_pool_mutation_and_nonfinite():
    ...
    broken[1]["raw_candidate_ids"] = ["bad"] * 12
